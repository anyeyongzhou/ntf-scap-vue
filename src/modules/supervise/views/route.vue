	...
	<cl-crud ref="Crud">
		...
		<cl-upsert ref="Upsert">
			<template #slot-boundary="{ scope }">
				<layer-draw v-model="scope.boundary" type="1" />
			</template>
		</cl-upsert>
	</cl-crud>
</template>

<script lang="ts" name="supervise-route" setup>
...
const Upsert = useUpsert({
	items: [
		{ label: "航线名称", prop: "name", required: true, component: { name: "el-input" } },
		{
			label: "航线颜色",
			prop: "color",
			required: true,
...
const Table = useTable({
	columns: [
		...
		{ label: "航线颜色", prop: "color" },
		{ label: "航线描述", prop: "description" },
...
const Crud = useCrud(
	{
		service: service.supervise.route
	},
	(app) => {
		app.refresh();
	}
